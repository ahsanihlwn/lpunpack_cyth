...
def build_attribute_string(int attributes):
...
class SparseImage:
    ...
    def unsparse(self):
        if not self.header:
            self._fd.seek(0)
            self.header = SparseHeader(self._fd.read(SPARSE_HEADER_SIZE))
        cdef int chunks = self.header.total_chunks
        cdef int sector_size
        cdef int chunk_data_size
        ...
        with open(str(unsparse_file), 'wb') as out:
            ...
            output_len = 0
            while chunks > 0:
                hdr = self._fd.read(SPARSE_CHUNK_HEADER_SIZE)
                chunk_type, _, chunk_sz, total_sz = struct.unpack("<2H2I", hdr)

                sector_size = (chunk_sz * self.header.blk_sz) >> 9
                chunk_data_size = total_sz - self.header.chunk_hdr_sz
...
class LpUnpack(object):
    ...
    def _get_data(self, int count, int size, clazz):
        cdef list result = []
        cdef int i
    ...
    def _read_chunk(self, int block_size):
        cdef object fd = self._fd
        cdef bytes data
        while True:
    ...
    def _write_extent_to_file(self, fd, int offset, int size, int block_size):
        cdef object src = self._fd
        cdef bytes block
